...
export const getRelativeTime = (timeStamp, locale) => {
  ...
  // 少于等于59秒
    ...
    resStr =
      parseInt(diff / 1000) + (locale("i.time.seconds") || "秒") + dirStr;
  // 多于59秒，少于等于59分钟59秒
  else if (diff >= 60000 && diff < 3600000)
    resStr =
      Math.floor(diff / 60000) + (locale("i.time.minutes") || "分钟") + dirStr;
  // 多于59分钟59秒，少于等于23小时59分钟59秒
  else if (diff >= 3600000 && diff < 86400000)
    resStr =
      Math.floor(diff / 3600000) + (locale("i.time.hours") || "小时") + dirStr;
  // 多于23小时59分钟59秒，少于等于29天59分钟59秒
  else if (diff >= 86400000 && diff < 2623860000)
    resStr =
      Math.floor(diff / 86400000) + (locale("i.time.days") || "天") + dirStr;
